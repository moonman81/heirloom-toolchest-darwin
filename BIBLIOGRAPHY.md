...
historical texts that inform the `toolchest` package (POSIX/SVR4 core utilities) in
...
- Aho, Alfred V., Peter J. Weinberger and Brian W. Kernighan
  (1988). *The AWK Programming Language*. Addison-Wesley.
  Foundation for `nawk` and `oawk` in this package.
- McIlroy, M. D. (1987). "A Research UNIX Reader." Bell Labs.
  Documents the design of many toolchest utilities.
- Ritchie, Dennis M. and Ken Thompson (1974). "The UNIX Time-
  Sharing System." Communications of the ACM 17(7). Contextual
  reference for the tool set as a whole.
- IEEE POSIX.1-2001 utilities specification — the canonical
  behaviour target for the `posix2001/` variants.
- Apple Inc. libproc man pages (proc_listpids(3), proc_pidinfo(3))
  — foundation for the Darwin `ps`, `pgrep`, and `whodo` rewrites.
- Mach umbrella framework (<mach/mach.h>) — task_info(),
  thread_info(), mach_port_deallocate() references used by the
  process-enumeration utilities on Darwin.
