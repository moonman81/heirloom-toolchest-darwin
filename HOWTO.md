# HOWTO — install and use Heirloom Toolchest on Darwin
...
```sh
git clone https://github.com/moonman81/heirloom-toolchest-darwin
cd heirloom-toolchest-darwin
```
...
Gunnar Ritter maintained. Nothing in `heirloom-toolchest-darwin` alters
...
writes `.install-manifest-toolchest.txt` at the repo root. That
...
Reads `.install-manifest-toolchest.txt`, removes every listed file
